...
# ---------------- UI ----------------
st.title("💰 Hisab Software V2")
st.caption("Clean line-by-line accounting • Bank Statement Upload • Online/Cash tracking • Party reports")
    ...
    if up is not None:
        ...
        if err:
            ...
            if normalized.empty:
                ...
                st.dataframe(review[display_cols], use_container_width=True, hide_index=True)
...
with tabs[2]:
    ...
    if txt:
        ...
        if row:
            ...
            edited = st.data_editor(preview, use_container_width=True)
...
# ---------------- Party Report ----------------
with tabs[4]:
    st.subheader("Party-wise Report")
    df = load_transactions()
